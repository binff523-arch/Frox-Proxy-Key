...
```
frox-proxy/
├── app.py              # Toàn bộ code: Flask backend + giao diện HTML nhúng sẵn
├── keys.json           # Danh sách 100 key
├── requirements.txt    # Thư viện cần cài
├── Procfile             # Lệnh khởi chạy cho Render
└── render.yaml          # Cấu hình deploy tự động (tùy chọn)
```
Toàn bộ giao diện (trang chủ, trang hiện key, trang báo lỗi) đã được gộp thẳng vào `app.py` — không cần thư mục `templates/` riêng, giúp việc tạo/sửa file trên GitHub (đặc biệt trên điện thoại) đơn giản hơn.
